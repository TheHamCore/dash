mapbox_access_token = 'pk.eyJ1Ijoic2VtbGFkIiwiYSI6ImNrenBiMXpzZzNmcmkyb255b3dleDlrbGoifQ.mb2gpUyl7Rvpw53-w6cCmQ'
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            ...
            html.Label(children=['Borougwwh: '], style=blackbold),
            dcc.Checklist(id='boro_name',
                          options=[{'label': str(b), 'value': b} for b in sorted(df['boro'].unique())],
                          value=[b for b in sorted(df['boro'].unique())],
                          ),
...
@app.callback(Output('graph', 'figure'),
              [Input('boro_name', 'value'),
               ...
               Input('input_input', 'value')])
def update_figure(chosen_boro, chosen_recycling, inp):
    #print(chosen_boro, 444444444)
    df_sub = df[(df['boro'].isin(chosen_boro)) &
                (df['type'].isin(chosen_recycling))]
    #print(df['boro'])

    # n = np.array(df_sub).tolist()
    # for v in n:
    #     print(v)
    #
    # print(n)
